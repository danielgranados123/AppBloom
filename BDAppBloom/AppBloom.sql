...
//Lleva datos de ejemplo para que pueda visualizar todas las herramientas de la aplicaci�n, y que la pueda ver funcionando de una forma m�s realista.
...
//Lo guardo como tipo de dato string/varchar, ya que en algunos lugares las habitaciones se identifican con claves alfanumericas (Habitaci�n 1A, 2B, etc)
...
----Tabla para almacenar los n�meros de cama en cada habitaci�n---- 
...
/*Interpreto que en los hospitales se utiliza una misma 'clave' para identificar las habitaciones y las camas. Por ejemplo: En las habitaciones A1, B2, C3, a pesar de ser distintas, es posible que todas tengan el mismo n�mero de camas. Por lo tanto: Habitaci�n A1 - Camas 1, 2, 3, 4, 5 Habitaci�n B2 - Camas 1, 2, 3, 4, 5 Habitaci�n C3 - Camas 1, 2, 3, 4

Y de este modo, se genera una relaci�n de muchos a muchos, ya que
una misma habitaci�n puede tener varias camas, y cada n�mero de cama (que es
...
----Tabla para guardar los pacientes con su informaci�n principal---- 
...
insert all into tbEnfermedades (nombre_enfermedad) values ('Neumon�a') into tbEnfermedades (nombre_enfermedad) values ('Epilepsia') into tbEnfermedades (nombre_enfermedad) values ('Quemaduras Primer Grado') into tbEnfermedades (nombre_enfermedad) values ('Quemaduras Segundo Grado') into tbEnfermedades (nombre_enfermedad) values ('Quemaduras Tercer Grado') into tbEnfermedades (nombre_enfermedad) values ('C�ncer') into tbEnfermedades (nombre_enfermedad) values ('Apendicitis') into tbEnfermedades (nombre_enfermedad) values ('Traumatismo') into tbEnfermedades (nombre_enfermedad) values ('Diabetes Tipo 1') into tbEnfermedades (nombre_enfermedad) values ('Diabetes Tipo 2') into tbEnfermedades (nombre_enfermedad) values ('Asma') select * from dual;

select * from tbEnfermedades;

insert all into tbMedicamentos (nombre_medicamento) values ('Amoxicilina') into tbMedicamentos (nombre_medicamento) values ('Ceftriaxona') into tbMedicamentos (nombre_medicamento) values ('Azitromicina') into tbMedicamentos (nombre_medicamento) values ('Paracetamol') into tbMedicamentos (nombre_medicamento) values ('Ibuprofeno') into tbMedicamentos (nombre_medicamento) values ('Morfina') into tbMedicamentos (nombre_medicamento) values ('Insulina lispro') into tbMedicamentos (nombre_medicamento) values ('Insulina aspart') into tbMedicamentos (nombre_medicamento) values ('Insulina glargina') into tbMedicamentos (nombre_medicamento) values ('Insulina detemir') into tbMedicamentos (nombre_medicamento) values ('Fenito�na') into tbMedicamentos (nombre_medicamento) values ('Fentanilo') select * from dual;

select * from tbMedicamentos;

insert all 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('FAA6C2017A20429395E3A6E94EBF55F4', 'Daniel Isaac', 'Granados Ca�as', 4, 1) 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('990FCF6268BA4B3B9C3D2EF16E15C8AE', 'Bryan Exequiel', 'Miranda Rodr�guez', 5, 7) 
...
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('69352A5EDCF54B718896ABF6FEB2C22E', 'Emerson Alexander', 'Gonz�lez Rodr�guez', 4, 19)

into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('5F7284144F664B95B70BF652A24F41AA', 'Manuel Alejandro', 'Ortega Rodr�guez', 6, 25) 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('98D389BEB24D4AFD97B184E7E22BD6B6', 'Bryan Eduardo', 'Cornejo P�rez', 7, 30) 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('3BC6A55BCDA8462AA9A058CF87FFB325', 'Rodrigo Andr�s', 'Hurtado Quezada', 8, 35) 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('07A4A2E84B9A4E43A46C85098788E4CC', 'Rodrigo Josu�', 'Monterrosa Jorge', 9,40)
...
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('400F7DB029F24CEF827EC9A612616B94', 'Edenilson Alexander', 'Amaya Ben�tez', 11, 53) 
into tbPacientes (id_paciente, nombres_paciente, apellidos_paciente, edad_paciente, ID_HabitacionCama) values ('42ED3DB064794EAC94F07E45ABEFB2B0', 'Jos� Luis', 'Iraheta Marroqu�n', 12, 57) 
select * from dual;
